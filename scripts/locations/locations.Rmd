...
```{r}
#field corrections to be handles in MAXQDA
# field_corrections %<>% 
...
missing_locs <- unique(segments_db$study_id[!segments_db$study_id %in% unique(dat$study_id)])
length(missing_locs)

#Studies without study location
study_locs <- unique(segments_db$study_id[segments_db$code_main_cat=="location"])
no_study_locs <- unique(segments_db$study_id[!segments_db$study_id %in% study_locs])
load(here("data", "articles_db.RData"))
sub_index <- articles_db %>%
  filter(study_id %in% no_study_locs) %>%
  select(study_id, mex_name)

#gs_new("amr_db_missing_locations_study_id", input=sub_index)
...
test_segments_db <- segments_db %>% filter(study_id %in% code_studies)
