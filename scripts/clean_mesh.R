...
mesh <- mesh0 %>%
  ...
  rename_all(funs(paste0('drug_', .)))  %>%
  rename(segment = drug_segment, drug_id = drug_class_id)

# filter out synonyms that are also preferred labels
pref_names <- mesh %>%
  filter(drug_segment_name_class == "preferred_label") %>%
  pull(segment) %>%
  unique()

mesh %<>% filter(!c(segment %in% pref_names & drug_segment_name_class == "synonyms"))
