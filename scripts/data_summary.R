...
heatmap <- ggplot(heatmap_mat %>% 
  ...
  geom_text(aes(label = n), color = "darkslategray") +
...
drugs_total <- top_drugs %>% 
  ...
  geom_text(aes(label=lab), y = 2, hjust = "left", color = "coral2") +
  scale_fill_viridis_c(guide = FALSE, alpha = 0.5) +
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0), position = "right") +
  labs(x = "", y = "Total AMR Events") +
  coord_flip() + 
  theme_bw() +
...
bact_total <- top_bact %>% 
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label=lab), y = -5, angle = 90, hjust = 1, color = "coral2") +
  scale_fill_viridis_c(guide = FALSE, alpha = 0.5) +
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_reverse(expand = c(0, 0)) +
  labs(y = "Total AMR Events", x = "") +
  theme_bw() +
...
ggsave(here("figures/drug-bacteria-combos.png"), width = 17, height = 9)
